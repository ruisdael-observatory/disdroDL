...
def test_db_insert():
    ...
    for i in res.fetchall():
        ...
        datetime_as_dt = datetime.strptime(datetime_.split('+')[0],
...
def test_unpack_telegram_from_db():
    """
    test unpacking telegram from db
    """
...
def test_query_db():
    ...
    test query db
    """
    delete_netcdf(fn_start='test', data_dir_=data_dir, )  # delete old netCDF
    telegram_objs = []
    con, cur = connect_db(dbpath=str(db_path))
    res = cur.execute("SELECT COUNT(*) FROM disdrodl;")
...
def test_NetCDF():
    ...
    assert (netCDF_var_temp_r_sensor_data[0]
            == netCDF_var_temp_l_sensor_data[0])
...
def delete_netcdf(fn_start, data_dir_):
    """
    def delete netCDF
    :param fn_start:
    :param data_dir_:
    """
    test_nc_path = data_dir_ / f'{fn_start}.nc'
    if os.path.exists(test_nc_path):
        os.remove(test_nc_path)


@pytest.fixture()
def db_insert_24h_w_gaps():
    """
    def db insert 24h with gaps
    """
