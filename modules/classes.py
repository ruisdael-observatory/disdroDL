...
class NetCDF:
    ...
    def create_netCDF(self):
        ...
        self.logger.info('class NetCDF executed create_netCDF()')
    ...
    def write_data_to_netCDF(self):
        ...
        for key in self.telegram_objs[0].telegram_data.keys():
            if key in self.config_dict['telegram_fields'].keys() and \
                ...
                logger.debug('Handling values from NetCDF var: ',
                             key,
                             netCDF_var.standard_name,
                             netCDF_var.dtype,
                             netCDF_var._vltype,
                             netCDF_var._isvlen,
                             'dims:', netCDF_var._getdims()
                             )
        ...
        netCDF_rootgrp.close()
        self.logger.info('class NetCDF executed write_data_to_netCDF()')
